...
public class DailyExerciseInfo extends AppCompatActivity {
    ...
    public void onViewClicked() {
        for (int i = 0; i < mEXList.size(); i++) {
            if (!mEXList.get(i).isOnline()) {
                isMove = false;
                int finalI = i;
                Glide.with(context).load(mEXList.get(i).getUrl()).addListener(new RequestListener<Drawable>() {
                    @Override
                    public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean isFirstResource) {
                        mEXList.get(finalI).setOnline(false);
                        return false;
                    }

                    @Override
                    public boolean onResourceReady(Drawable resource, Object model, Target<Drawable> target, DataSource dataSource, boolean isFirstResource) {
                        mEXList.get(finalI).setOnline(true);
                        return false;
                    }
                });
            }
    ...
    private class getTask extends AsyncTask<Void, Void, Void> {
        ...
        @Override
        protected final Void doInBackground(Void... params) {
            AppDataBase.getInstance().exerciseDao().insertAll(mEXList);
            return null;
        }
